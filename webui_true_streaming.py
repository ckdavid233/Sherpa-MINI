#!/usr/bin/env python3
"""
sherpa-onnx HTTPS WebSocket 真正原生流式识别
使用 Zipformer 原生流式模型，每次音频块直接解码，响应超快
"""
...
def init_recognizer():
    ...
    print("正在加载 Zipformer 原生流式模型 (GPU 加速)...")
    print(f"模型: {model_dir} (INT8 量化)")
    ...
    print("原生流式模型加载完成！支持：中文/英文 流式识别")
    return recognizer
...
def index():
    """主页"""
    return render_template('true-streaming.html')
...
def status():
    ...
    return json.dumps({
        ...
        "model_name": "Zipformer 原生流式"
    })


@sock.route('/ws/stream')
def stream_asr(ws):
    """WebSocket 原生流式识别端点 - 每个音频块直接解码"""
    ...
    try:
        while True:
            ...
            if msg_type == 'start':
                ...
                if samples is not None and recognizer is not None:
                    # 原生流式：直接送入，立刻解码
...
def main():
    ...
    parser = argparse.ArgumentParser(description="sherpa-onnx HTTPS 原生流式 ASR 服务")
    parser.add_argument("--host", type=str, default="0.0.0.0", help="监听地址")
    parser.add_argument("--port", type=int, default=6009, help="监听端口")
...
<head>
    ...
    <title>sherpa-onnx 原生流式语音识别</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        <h1>⚡ 原生流式语音识别</h1>
        <p class="subtitle">基于 sherpa-onnx + Zipformer · GPU 加速 · 真正实时</p>

        <div class="model-badge">
            <span>✓ 原生流式 · 超低延迟 · 边说边出</span>
        </div>
        ...
        <div class="stats" id="stats" style="display: none;">
            ...
            <div class="stat-card">
                <div class="stat-value" id="latency">&lt;10ms</div>
                <div class="stat-label">每块延迟</div>
            </div>
            <div class="stat-card">
        ...
        <div class="result-container" style="margin-top: 25px;">
            <div class="result-label">实时识别结果（边说边更新）</div>
            <div class="result-text empty" id="resultText">点击"开始识别"开始说话...</div>
        </div>

        <div class="info-box">
            <p><strong>特点：</strong>Zipformer 是原生流式模型，每个音频块进来直接解码，真正实时输出。GPU 加速下每块延迟都在 10ms 以内，文字秒出。支持中英文双语。</p>
        </div>
    </div>
    ...
    <script>
        ...
        function connect() {
            ...
            ws.onopen = () => {
                statusText.textContent = '已连接 - 原生流式就绪';
        ...
        async function startRecording() {
            try {
                ...
                // 创建 ScriptProcessorNode 处理音频 - 每 4096 个样本回调一次
                ...
                processor.onaudioprocess = (e) => {
                    ...
                    latencyEl.textContent = avgLatency.toFixed(1) + 'ms';
                };
                ...
                statusText.textContent = '正在识别中... 边说边出结果';
    ...
    with open(templates_dir / "true-streaming.html", "w") as f:
        f.write(index_html)
    ...
    if not args.no_ssl and not Path(args.cert).exists():
        ...
        try:
            ...
            cert.get_subject().CN = "sherpa-onnx-true-streaming"
            cert.set_serial_number(4000)
    ...
    print("\n" + "=" * 70)
    print("  sherpa-onnx HTTPS 原生流式 ASR 服务已启动")
    print("=" * 70)
    print(f"\nModel: Zipformer (原生流式 INT8)")
    print(f"Supported languages: 中文 / 英文")
    print(f"\n请在浏览器中访问以下地址之一:")
    protocol = "http" if args.no_ssl else "https"
    print(f"  - 本机: {protocol}://localhost:{args.port}")
    print(f"  - 局域网: {protocol}://{server_ip}:{args.port}")
    print(f"\n✅ 特点:")
    print(f"  - 真正原生流式 - 每个音频块直接解码")
    print(f"  - 超低延迟 - GPU 每块延迟通常 < 10ms")
    print(f"  - 实时输出 - 边说话边更新文字")
    print(f"  - GPU 加速 (CUDA)")
