...
#
# 用法:
#   ./start_vad_sensevoice.sh                   # 默认 6020 端口
...
# 6. 监听端口
# ============================================
# 监听端口：默认 6020；若端口被占用可换端口，例如: PORT=6011 ./start_vad_sensevoice.sh
#
# 为何重启后仍提示 6020 被占用、且 ss/lsof 里像 ssh 在占？
#   - 用 Cursor/VS Code Remote SSH 时，若在「端口转发 / Ports」里转发过 6020，sshd 会在板子上
#     监听该端口（或显示为 ssh 相关），与你想再 bind 的 Web 服务冲突。应在 IDE 里删除该转发，
#     或换用未转发的端口访问（例如只在内网浏览器打开 https://板子IP:6020，不要从本机转发同号端口）。
