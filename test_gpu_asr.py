...
# 添加当前目录到路径
sys.path.insert(0, '/home/tsingwin/apps/sherpa-onnx')
...
def create_recognizer(use_gpu=True):
    """创建识别器"""
    ...
    print(f"Creating recognizer with provider: {provider}")
...
def main():
    ...
    try:
        recognizer_gpu = create_recognizer(use_gpu=True)
        ...
        inference_time_gpu = test_recognizer(recognizer_gpu)
    ...
    try:
        recognizer_cpu = create_recognizer(use_gpu=False)
        ...
        inference_time_cpu = test_recognizer(recognizer_cpu)
